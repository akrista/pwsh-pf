...
- Downloads the `lambdageneration.omp.json` theme into your PowerShell profile directory.
- Installs extra CLI tools (bat, gsudo, ripgrep, fd, gitui) via Scoop.
...
```powershell
$Home\lambdageneration.omp.json
```
...
You can also place `custom.ps1` beside `Microsoft.PowerShell_profile.ps1`. It is loaded before this profile reads override variables and functions.
