### Chris Titus Tech's PowerShell profile (fork)
...
############                       https://github.com/akrista/pwsh-pf.git.                         ############
############                                                                                                         ############
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!#
############                                                                                                         ############
...
$script:CustomProfile = Join-Path -Path $script:ProfileRoot -ChildPath 'custom.ps1'
...
$repo_root = if ($repo_root_Override) { $repo_root_Override } else { 'https://raw.githubusercontent.com/akrista' }
...
function Update-Profile {
    ...
    $url = "$repo_root/pwsh-pf/master/Microsoft.PowerShell_profile.ps1"
...
function Resolve-OhMyPoshTheme {
    $candidates = @(
        $env:POSH_THEME,
        (Join-Path $profileDir 'lambdageneration.omp.json'),
        (Join-Path $HOME 'lambdageneration.omp.json')
...
function Initialize-PromptTool {
    ...
    if (Get-Command -Name 'Get-Theme_Override' -ErrorAction SilentlyContinue) {
        ...
        if (-not $theme) {
            $theme = Join-Path $profileDir 'lambdageneration.omp.json'
            try {
                Save-UriToFile -Uri 'https://raw.githubusercontent.com/akrista/.akrista/refs/heads/master/lambdageneration.omp.json' -OutFile $theme
            } catch {
                Write-Warning "Failed to download the lambdageneration theme. Error: $_"
                $theme = $null
            }
        }
        if ($theme) {
            ...
            Write-Warning 'Oh My Posh theme not found. Run setup.ps1 to install lambdageneration.omp.json.'
        }
    } else {
...
Shortcuts:
  ...
  hb <file>         Upload a file to hastebin and copy the URL.
